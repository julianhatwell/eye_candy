...
getCost <- function(expr) {
  fmla <- eval(expr)
  if (length(attr(terms(fmla), "variables")) < 3) { return(Inf) }
  
  model <- gam(fmla, data = trn)
  if (is.null(model)) { return(Inf) }
  
  test.results <- predict(model, tst)
  cost <- mean((test.results - tst$PRP)^2)
  return(cost)
}
...
ge
gam.fmla <- eval(ge$best$expressions)
fit <- gam(gam.fmla, data = computer.data.log)
summary(fit)
